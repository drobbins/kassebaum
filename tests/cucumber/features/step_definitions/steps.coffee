...
module.exports = ->
    ...
    @.Given /an alert with "([^"]*)" should be displayed/, (text, next) ->
        @browser.waitForVisible alertXpath(text)
            .call next
    ...
    @.When /^I click on the "([^"]*)" patient link for patient "([^"]*)"$/, (operation, shortId, next) ->
        ...
        @browser.click linkTitleXpath linkTitle
            .call next
    ...
    @.When /^enter and submit a new username and password$/, (next) ->
        @ddp.call "/fixtures/removetestuser", Data.newUser.username, (err, result) =>
    ...
    @.Then /^I should be logged in as this new user$/, (next) ->
        @browser.waitForExist linkXpath Data.newUser.username
            .call next